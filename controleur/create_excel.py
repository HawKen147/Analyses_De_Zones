...
def format_excel(path_to_folder):
    wb = Workbook()
    ...
    for cols in ws.iter_cols(min_col=3, max_col=20):
        ...
        if number_odd % 2 == 1:
            ws.column_dimensions[col_letter].width = 29
        else:
    ...
    nb_camera = fonctions.get_nb_camera(path_to_folder) + 5
    ...
    ##Applique le style a la dernier ligne
    for cell in ws.iter_rows(min_row=nb_camera, max_row=nb_camera, min_col=1, max_col=20):
